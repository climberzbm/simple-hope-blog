...
export default function PostCard({ post }: Props) {
  return (
    <Link href={`/posts/${post.slug}`} className="block group bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-200">
      {post.cover && (
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="flex gap-1.5 overflow-hidden">
              {post.tags.slice(0, 2).map((tag) => (
                <span key={tag.id} className="truncate">#{tag.name}</span>
              ))}
            </div>
          )}
        </div>
